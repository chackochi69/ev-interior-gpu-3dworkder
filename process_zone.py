...
def run_colmap_pipeline(work_dir: str, img_dir: str) -> str:
    """Run COLMAP via pycolmap Python API. Returns path to sparse PLY."""
    import pycolmap

    log(f"pycolmap version: {getattr(pycolmap, '__version__', 'unknown')}")
    ...
    log(f"Images available for reconstruction: {len(img_files)}")

    log("Extracting features (CPU SIFT)...")
    # IMPORTANT: pycolmap bundles its own CPU-only COLMAP binary regardless of
    # the system COLMAP or CUDA setup. Always use CPU device to avoid silent
    # zero-feature extraction when GPU SIFT is unavailable in the wheel.
    _reader_options = pycolmap.ImageReaderOptions()
    _reader_options.camera_model = "SIMPLE_RADIAL"

    # Build extraction options — handle pycolmap 0.x (FeatureExtractionOptions)
    # and 3.x (SiftExtractionOptions) APIs.
    _sift_opts = None
    for cls_name in ("SiftExtractionOptions", "FeatureExtractionOptions"):
        cls = getattr(pycolmap, cls_name, None)
        if cls is not None:
            _sift_opts = cls()
            log(f"Using {cls_name} for SIFT options")
            break

    if _sift_opts is not None:
        # Always CPU — pycolmap wheel does not include GPU SIFT
        for attr in ("use_gpu", "gpu_index"):
            try:
                setattr(_sift_opts, attr, False if attr == "use_gpu" else -1)
            except AttributeError:
                pass
        for attr in ("num_threads",):
            try:
                setattr(_sift_opts, attr, 2)
            except AttributeError:
                pass
        for attr in ("max_num_features",):
            try:
                setattr(_sift_opts, attr, 8192)
            except AttributeError:
                pass

    # Try pycolmap 3.x API first (sift_options=), fall back to 0.x (extraction_options=)
    extract_kwargs = dict(database_path=db_path, image_path=img_dir,
                          reader_options=_reader_options)
    if _sift_opts is not None:
        extract_kwargs["sift_options"] = _sift_opts
    try:
        pycolmap.extract_features(**extract_kwargs)
    except TypeError:
        # 0.x used extraction_options=
        extract_kwargs.pop("sift_options", None)
        if _sift_opts is not None:
            extract_kwargs["extraction_options"] = _sift_opts
        pycolmap.extract_features(**extract_kwargs)

    log("Matching features (exhaustive, CPU)...")
    # pycolmap wheel has no CUDA matching — always CPU.
    try:
    ...
    # Relax all thresholds for small image sets (4–20 images).
    # In both 0.x and 3.x, setting unknown attributes is a silent no-op,
    # so we set every plausible name for each setting.
    for attr, val in [
        ("min_model_size", 3),
        ("min_num_matches", 10),
        ...
        try:
            setattr(_mapping_opts, attr, val)
        except Exception:
            pass
    # Nested mapper options (attribute name consistent across versions)
    try:
        _mapping_opts.mapper.init_min_num_inliers = 20
        _mapping_opts.mapper.abs_pose_min_num_inliers = 10
        _mapping_opts.mapper.init_min_tri_angle = 2.0
    except AttributeError:
        pass
    ...
    best = maps[best_model_idx]
    log(f"Best model: {len(best.images)} images, {len(best.points3D)} 3D points")

    if len(best.points3D) == 0:
        raise RuntimeError(
            f"Reconstruction registered {len(best.images)} image(s) but triangulated "
            "0 3D points. Images may have too little overlap or texture. "
            "Try capturing more images with >60% overlap."
        )
    ...
    # Dense reconstruction is slow (5-30 min) and requires a long execution
    # timeout on the RunPod endpoint. Disabled by default; set
    # COLMAP_DENSE=1 as an environment variable on the endpoint to enable it.
...
def ply_to_glb(ply_path: str, output_glb: str) -> None:
    ...
    if isinstance(scene_or_mesh, trimesh.PointCloud):
        ...
        # Export the raw colored point cloud as a GLTF POINTS primitive.
        # Three.js GLTFLoader will load this as THREE.Points which the viewer
        # knows how to render with vertex colors.
